    ...
    while parent_state is not None:
        if current_state.leaves:
            if current_state.index == len(variant):
                leaf_labels = list(reversed([o.label for o in current_state.leaves]))
                ...
                alignment.append((variant[parent_state.index], current_state.leaves[-1]))
...
def _obtain_leaves_from_state_path(path, include_tau=False):
    if include_tau:
        return [t[0] for t in path if t[0].operator is None and t[1] == ProcessTree.OperatorState.OPEN]
    else:
        return [t[0] for t in path if t[0].operator is None and t[0].label is not None and t[1] == ProcessTree.OperatorState.OPEN]


def _need_log_move(old_state, new_state, path) -> bool:
    if _obtain_leaves_from_state_path(path, include_tau=True):
        return True
    choices = [o for o in old_state.keys() if pt_util.is_any_operator_of(o[1], [Operator.XOR, Operator.LOOP])]
    for choice_tuple in choices:
        choice = choice_tuple[1]
        if pt_util.is_operator(choice, Operator.XOR):
            if (pt_util.is_in_state(choice, ProcessTree.OperatorState.FUTURE, old_state)
                or pt_util.is_in_state(choice, ProcessTree.OperatorState.CLOSED, old_state)) \
                    and old_state[(id(choice), choice)] != new_state[(id(choice), choice)]:
                return True
        elif pt_util.is_operator(choice, Operator.LOOP):
            for loop_child in choice.children:
                if (pt_util.is_in_state(loop_child, ProcessTree.OperatorState.FUTURE, old_state)
                    or pt_util.is_in_state(loop_child, ProcessTree.OperatorState.CLOSED, old_state)) \
                        and old_state[(id(loop_child), loop_child)] != new_state[(id(loop_child), loop_child)]:
                    return True
    return False
...
def align_variant(variant, tree_leaf_set, pt):
    ...
    while open_set:
        ...
        if _is_final_tree_state(sga_state.state, pt) and sga_state.index == var_len:
            ...
            if sga_state.index < len(lmcf):
                ...
                candidates = [t for (i, t) in tree_leaf_set if t.label == current_label]

                need_log_move = (len(candidates) == 0)
                for leaf in candidates:
                    path, new_state_st = pt_sem.shortest_path_to_enable(leaf, copy.copy(sga_state.state))
                    if path is None:
                        need_log_move = True
                    else:
                        model_moves = _obtain_leaves_from_state_path(path)
                        # Check if we need a log move due to XOR/LOOP transitions in the path
                        need_log_move = need_log_move if need_log_move else _need_log_move(sga_state.state, new_state_st, path)
                        sync_path, new_state_st = pt_sem.shortest_path_to_close(leaf, new_state_st)
                        ...
                        new_state = SGASearchState(sga_state.costs + len(model_moves),
                                                   current_index + 1,
                                                   new_state_st, leaves=leaves, parent=sga_state)
                        new_state.path = tuple(path)
                        open_set, closed_set = _add_new_state(new_state, sga_state, open_set, closed_set)

                if need_log_move:
                    # Add a log move state
                    log_move_state = SGASearchState(sga_state.costs + lmcf[current_index],
                                                    current_index + 1,
                                                    copy.copy(sga_state.state), parent=sga_state)
                    open_set, closed_set = _add_new_state(log_move_state, sga_state, open_set, closed_set)
            else:
                # FINISH
                path, new_state_st = pt_sem.shortest_path_to_close(pt, sga_state.state)
...
def _construct_progress_bar(progress_length, parameters):
    if exec_utils.get_param_value(Parameters.SHOW_PROGRESS_BAR, parameters, constants.SHOW_PROGRESS_BAR) \
        ...
        with ProcessPoolExecutor(max_workers=num_cores) as executor:
            ...
            if progress is not None:
                while alignments_ready != len(futures):
                    current = sum(1 for variant in futures if futures[variant].done())
